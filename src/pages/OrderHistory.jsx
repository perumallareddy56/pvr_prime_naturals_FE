...
const OrderCard = ({ order, getStatusColor, getStatusBadge, handleDeleteOrder }) => {
  ...
  return (
    <Card key={order.id}
              ...
              <div className="mb-4 border-top border-white border-opacity-10 pt-4">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  {order.status === 'DELIVERED' && (
                    ...
                    >
                      {showMap ? 'CLOSE MAP' : 'VIEW DELIVERY ROUTE'}
                    </Button>
                  )}
                </div>
      ...
      <Card.Footer className="border-top border-white border-opacity-10 py-3 px-4 d-flex justify-content-between align-items-center" style={{ background: 'rgba(0,0,0,0.1)' }}>
        ...
        <Button variant="link" className="text-danger p-0 text-decoration-none small fw-bold opacity-75 hvr-grow" onClick={() => handleDeleteOrder(order.id)}>
          DELETE FROM HISTORY
        </Button>
      </Card.Footer>
    </Card>
...
const OrderHistory = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const historyOrders = res.data.filter(o => o.status === 'DELIVERED' || o.status === 'CANCELLED');
    ...
    <div className="orders-page bg-dark min-vh-100 py-5">
      <Container>
        <div className="text-center mb-5 reveal">
          ...
          <h2 className="display-4 fw-bold text-white">Order History</h2>
          ...
          <div className="d-inline-block px-4 py-2 rounded-pill border border-warning border-opacity-20 shadow-sm mt-4" style={{ background: 'rgba(255,255,255,0.02)' }}>
            <span className="fw-bold text-gold">{orders.length}</span> <span className="text-muted small letter-spacing-1 ms-2">ORDERS PLACED</span>
          </div>
        </div>
